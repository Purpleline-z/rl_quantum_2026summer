...
from pairwise_active_learning_pipeline import Config, Experiment, PairRows, canonical_pair, seed_everything
from project_file_locations import CODE_ROOT, RESULT_ROOT
...
def frozen_selection(seed: int, selector: str) -> tuple[list[str], Path]:
    snapshot = RESULT_ROOT / "selected_summaries" / "budget_curve_5seed" / "manifests" / "frozen_budget100_selections.json"
...
def export_frozen_selections() -> Path:
    ...
    target = RESULT_ROOT / "selected_summaries" / "budget_curve_5seed" / "manifests" / "frozen_budget100_selections.json"
...
def generate_classifier2_audit(data_root: str | None = None) -> Path:
    ...
    legacy = CODE_ROOT / "classifier2" / "classifier_training_code" / "train_unified.py"
...
def run_classifier2_bridge(data_root: str | None = None, seed: int = 42) -> Path:
    ...
    legacy_root = CODE_ROOT / "classifier2" / "classifier_training_code"
