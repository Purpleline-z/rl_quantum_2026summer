...
from legacy_paths import (ABSOLUTE_CSV, CLASSIFIER2_ROOT, DATA_ROOT,
                          OUTPUT_ROOT, PAIRWISE_CSV, PRETRAINED_ENCODER)
...
def load_data(test_split=0.2, random_seed=42):
    ...
    if PAIRWISE_CSV.exists():
        ...
        fallback_csv = DATA_ROOT / "Quantum Label Data - Pairwise_Comparison.csv"
    ...
    bad_images = []
    abs_csv = ABSOLUTE_CSV if ABSOLUTE_CSV.exists() else DATA_ROOT / "Quantum Label Data - Absolute_Scoring (1).csv"
    if abs_csv.exists():
...
def train(epochs=30, batch_size=16, lr=1e-4, samples_per_epoch=500):
    ...
    # Save final model
    final_path = OUTPUT_ROOT / "final_model.pth"
...
def test_model(model_path=None):
    ...
    if model_path is None:
        model_path = OUTPUT_ROOT / "best_model.pth"
